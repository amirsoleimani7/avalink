...
export default function Properties() {
  return (
    <div className="w-full flex flex-col  px-[15%] max-xl:px-[10%] max-md:px-[8%] max-sm:px-[4%] select-none ">
      <div className="rounded-2xl border  w-full divide-x grid grid-cols-4 max-lg:grid-cols-2 max-lg:divide-y-0 max-lg:divide-x-0  max-sm:grid-cols-1 max-sm:divide-x-0 max-sm:divide-y overflow-hidden bg-[url(/img/bg/gggrain5.svg)] shadow-xs shadow-gray-950">
        {PropertiesData.map((propData: PropertiesType, index: number) => (
          <div
            ...
            key={index}
          >
            <propData.icon className="text-main-secondary w-1/2 h-1/2 mt-5 group-hover:text-white duration-200 ease-in-out transition-all group-hover:rotate-3" />
